...
def predict_ct(paper_id,predy=10):
	...
	fea = numpy.mat(paper_param['trainF'])
	ti = paper_param['trainS']
	theta = numpy.mat(param['theta'])

	trainT = 2015 - int(paper_param['year']) #int(tr[0,-1])
	tr = numpy.mat([x for x in ti if x <= trainT])
	...
	for t in range(trainT+1,trainT+predy+1): #t=T+1:T+predy
		...
		# USE SIMULATION
		delta_ct = theta*numpy.mat(fea).T/w1*(numpy.exp(-w1*(t-1))-numpy.exp(-w1*t)) + \
